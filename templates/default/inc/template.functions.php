...
function getNavbarItem($phrase) {
    ...
    foreach($cfg as $element) {
        if(!is_array($element)) continue;
        if(!$element['active']) continue;
        if($element['phrase'] === $phrase) return $element;
    }
    return null;
}

//phrase = menu_txt_7 (DESCARGAS)
//phrase = menu_txt_3 (REGISTER)
function getRegisterButton() {
	$cfg = loadConfig('navbar');
	if(!is_array($cfg)) return;
	$registerBtn = getNavbarItem('menu_txt_3');
	if($registerBtn) {
    	$link = ($registerBtn['type'] == 'internal' ? __BASE_URL__ . $registerBtn['link'] : $registerBtn['link']);
    	$title = lang($registerBtn['phrase'], true);
    	echo '<a href="'.$link.'" class="registerBtn">'.$registerBtn['icon'].' '.$title.'</a>';
	}
}
...
function getDownloadButton() {
	$cfg = loadConfig('navbar');
	if(!is_array($cfg)) return;
	$downloadBtn = getNavbarItem('menu_txt_7');
	if($downloadBtn) {
    	$link = ($downloadBtn['type'] == 'internal' ? __BASE_URL__ . $downloadBtn['link'] : $downloadBtn['link']);
    	$title = lang($downloadBtn['phrase'], true);
    	echo '<a href="'.$link.'" class="downloadBtn">'.$downloadBtn['icon'].' '.$title.'</a>';
	}
}
...
function templateBuildNavbar() {
	...
	foreach($cfg as $element) {
		if(!is_array($element)) continue;
		if (in_array($element['phrase'], ['menu_txt_3'])) {
    continue;
}
